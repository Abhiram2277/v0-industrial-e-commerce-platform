        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 md:gap-6">
          {industries.map((industry, index) => {
            ...
            return (
              <div
                key={index}
                className="flex flex-col items-center gap-2 md:gap-3 p-3 md:p-6 rounded-lg bg-background border hover:border-accent/50 hover:shadow-md transition-all hover:-translate-y-1"
              >
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-accent/10 flex items-center justify-center px-0">
                  <Icon className="w-5 h-5 md:w-6 md:h-6 text-accent px-px py-px mx-px my-0 border-0" />
                </div>
